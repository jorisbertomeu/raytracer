...
** Last update Sun Jun  8 01:41:15 2014 nicolas ades
*/
...
void		create_save_win(GtkWidget *parent, t_editor *editor)
{
  ...
  gtk_file_selection_hide_fileop_buttons(GTK_FILE_SELECTION(editor->pselection));
  gtk_window_set_position(GTK_WINDOW(editor->pselection), GTK_WIN_POS_CENTER);
  gtk_widget_show(editor->pselection);
  g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(editor->pselection)->ok_button), "clicked", G_CALLBACK(create_file), editor);
  g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(editor->pselection)->cancel_button), "clicked", G_CALLBACK(my_widget_destroy), editor);
